...
for b in $ALLOWED_BRANCHES; do
    if git ls-remote --exit-code --heads https://github.com/rarr111110-creator/caelestia-dots-kde-void.git "$b" >/dev/null 2>&1; then
...
if [ ! -d "$REPO" ]; then
    git clone --bare --filter=blob:none https://github.com/rarr111110-creator/caelestia-dots-kde-void.git "$REPO" >/dev/null 2>&1
else
...
def fetch_releases() -> list:
    url = "https://api.github.com/repos/rarr111110-creator/caelestia-dots-kde-void/releases?per_page=100"
