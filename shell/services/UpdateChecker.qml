...
for b in $ALLOWED_BRANCHES; do
    if git ls-remote --exit-code --heads https://github.com/rarr111110-creator/test.1.1.1.1rar111110.git "$b" >/dev/null 2>&1; then
...
if [ ! -d "$REPO" ]; then
    git clone --bare --filter=blob:none https://github.com/rarr111110-creator/test.1.1.1.1rar111110.git "$REPO" >/dev/null 2>&1
else
...
def fetch_releases() -> list:
    url = "https://api.github.com/repos/rarr111110-creator/test.1.1.1.1rar111110/releases?per_page=100"
